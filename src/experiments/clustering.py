...
MAX_DISTANCE = 0.2  # Max normalized distance between blocks to consider merging (0-1)
...
SHOW_PAGE_LAYOUT = True  # Whether to display page layout with groupings
...
def main():
    ...
    df = pd.read_csv(PROJECT_ROOT / 'ocr_with_boxes_unrolled_corrected.csv', converters={'rects': literal_eval})
    ...
    for page_num, page_df in df.groupby('Page'):
        ...
        if SHOW_GRAPH:
            visualize_connections(page_df, similarity_matrix, page_num)
        # break
...
def build_hybrid_similarity_matrix(df: pd.DataFrame, embeddings: np.ndarray, w_content=0.7, w_spatial=0.3) -> np.ndarray:
    ...
    spatial_dist = cdist(centers, centers, 'cityblock')
    ...
    for cluster in article_clusters:
        ...
        for title_idx in title_indices:
            ...
            if (title_rect[0][1] < max_y and
                    title_rect[0][0] >= min_x - (max_x - min_x) * MAX_DISTANCE and
                    title_rect[1][0] <= max_x + (max_x - min_x) * MAX_DISTANCE):
...
def visualize_page_layout(df: pd.DataFrame, clusters: list[dict[str, Any]], page_num: int):
    ...
    if 'original_paths' in df.columns:
        img_path = df['original_paths'].iloc[0]
        try:
            img = plt.imread((PROJECT_ROOT / "val_data" / img_path).parent.parent.parent.with_suffix(".jpg"))
...
def visualize_connections(df: pd.DataFrame, similarity_matrix: np.ndarray, page_num: int):
    """Visualize the connection graph based on similarity."""
    G = nx.Graph()

    # Add nodes
    for i, row in df.iterrows():
        G.add_node(i, label=f"{i}: {row['entity_types']}",
                   type=row['entity_types'], text=row['text'][:20] + "...")

    # Add edges for top similarities
    ...
    plt.title(f"Page {page_num} Content Similarity Graph")

    pos = nx.spring_layout(G, seed=42)
    node_colors = ['#ff7f0e' if df.iloc[i]['entity_types'] == 'title' else '#1f77b4'
                   for i in G.nodes()]
